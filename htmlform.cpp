...
//       NewValues->slInfoType->Add(pNode->strClassName);
		nIndex = NewValues->slInfoType->IndexOf(pNode->strControlName);
		if (nIndex < 0)
...
void THtmlForm::SetControlText(TInputInfo * NewValues)
{
    ...
    while (pNode)
	{
        ...
        if (pNode->strClassName == "TMemo")
        {
            ...
            pMemo->Clear();
            pMemo->Text = (AnsiString)((char *)NewValues->lInfo->Items[nCount]);
...
bool THtmlForm::CheckValues()
{
	...
	{
       if ((pNode->strClassName == "TEdit")
           || (pNode->strClassName == "TMaskEdit")
           || (pNode->strClassName == "TMemo"))
       {
            if (pNode->strClassName == "TEdit")
            {
                if (!((TEdit*)pNode->pControl)->Text.Trim().IsEmpty())
                    return true;
                else
                {
                	pNode = pNode->pNextControl;
            ...
            if (pNode->strClassName == "TMaskEdit")
            {
                if ((!((TMaskEdit*)pNode->pControl)->Text.Trim().IsEmpty())
            ...
            if (pNode->strClassName == "TMemo")
            {
                if (!((TMemo*)pNode->pControl)->Text.Trim().IsEmpty())
                    return true;
                else
                {
                	pNode = pNode->pNextControl;
...
bool THtmlForm::GetImage(TStream* pStream,AnsiString strFieldName)
{
    ...
    while (pNode)
	{
       ...
       {
             try{
			 ((TImageEn* )pNode->pControl)->IO->SaveToStreamJpeg(pStream);
			 }
             catch(...)
             {
             }
...
bool THtmlForm::SetTimeCode(AnsiString strTimeCode,AnsiString strFieldName)
{
    ...
    while (pNode)
	{
       ...
       {
            SetSingleControlText(strFieldName,strTimeCode.c_str());
            return true;
       }
       else
       {
...
//---------------------------------------------------------------------------
bool THtmlForm::SetSingleControlText(AnsiString FieldName,char * pNewValue)
{
	ControlInfo * pNode;
	TMemo * pMemo;
	TComboBox * pComboBox;
	...
	while (pNode)
	{
		if (FieldName == pNode->strControlName)
		{
			if (pNode->strClassName == "TEdit")
			{
